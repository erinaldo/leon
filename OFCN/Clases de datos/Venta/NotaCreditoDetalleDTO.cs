...
namespace OFC
{
    class NotaCreditoDetalleDTO
    {
        ...
        public static List<NotaCreditoDetalleDTO> obtenerDetalleCred(long idCredito)
        {
            List<NotaCreditoDetalleDTO> lista = new List<NotaCreditoDetalleDTO>();
            NpgsqlConnection cn = BaseDeDatos.obtenerConexion();
            IList<NpgsqlParameter> parameters = new List<NpgsqlParameter>();

            string sql = "select det.cantidad cantidad, det.precio_unitario pUnitario, det.importe importe, det.descripcion descripcion, det.cod_comprobante_factura compFactura";
            sql = sql + " from ofc_nota_credito credito, ofc_comprobante_temp c_temp, ofc_nota_credito_detalle det";
            sql = sql + " where credito.id = c_temp.id_origen";
            sql = sql + " and det.id_nota_credito = credito.id";
            sql = sql + " and c_temp.comprobante = 'NOTA CREDITO'";
            sql = sql + " and credito.id = @idCredito";
            ...
            {
                NotaCreditoDetalleDTO e = new NotaCreditoDetalleDTO();
                ...
                if (data["compFactura"] != null && data["compFactura"] != DBNull.Value)
                {
                    e.cod_comprobante_factura = data["compFactura"].ToString();
                }
        ...
        public static List<NotaCreditoDetalleDTO> obtenerDetalleReg(long idCredito)
        {
            List<NotaCreditoDetalleDTO> lista = new List<NotaCreditoDetalleDTO>();
            NpgsqlConnection cn = BaseDeDatos.obtenerConexion();
            IList<NpgsqlParameter> parameters = new List<NpgsqlParameter>();
            ...
            string sql = "select det.cantidad cantidad, det.precio_unitario pUnitario, det.importe importe, det.descripcion descripcion, det.cod_comprobante_factura compFactura";
            sql = sql + " from ofc_nota_credito credito, ofc_nota_credito_detalle det";
            sql = sql + " where det.id_nota_credito = credito.id";
            sql = sql + " and credito.id = @idCredito";
            sql = sql + " order by det.id";
